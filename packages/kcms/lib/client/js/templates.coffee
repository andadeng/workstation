...
Template.AdminDashboardView.rendered = ->
	...
	filter.html '
		<div class="input-group">
			<input type="search" class="form-control input-sm" placeholder=""></input>
...
Template.AdminDashboardSub.helpers
	hasDocuments1: ->
		AdminCollectionsCount.findOne({collection: Session.get 'admin_collection_name'})?.count > 0
	selector: -> 
...
Template.adminSubBtn.helpers
	path: ->
		Router.path "adminDashboard" + Session.get('admin_collection_name') + "Sub", _id: @_id

Template.userEmail.helpers
	username: ->
		Meteor.users.findOne(this.value).profile.username

Template.dateTime.helpers
	dateTime: ->
		moment(this.value).format("YYYY-MM-DD H:mm:ss")
